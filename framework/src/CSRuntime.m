...
@implementation CSRuntime

static CSRuntime* theRuntime = nil;

+(CSRuntime*)sharedRuntime {
	@synchronized (self) {
		if (theRuntime == nil) {
			[[CSRuntime alloc] init];
		}
	}
...
-init {
	[super init];
	NSString* classPath = [CSRuntime defaultClassPath];
	NSString* libraryPath = [CSRuntime defaultLibraryPath];
